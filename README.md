...
# Интеграция рекуператора Zentec Z031 для Home Assistant

Продвинутая конфигурация для полной интеграции приточно-вытяжной установки Zentec Z031 в экосистему Home Assistant. Этот проект позволяет не только отслеживать состояние рекуператора, но и полноценно управлять им, используя современный и удобный интерфейс.

Вся конфигурация упакована в единый файл-пакет для максимально простой установки.

*(**Рекомендация:** Сделайте скриншот вашей Lovelace-карточки и замените им ссылку выше, чтобы наглядно продемонстрировать результат)*
...
2.  Рекуператор **Zentec Z031**, подключенный к вашей локальной сети.
...
2.  Добавьте новую "Ручную" карточку и вставьте в нее [код из файла `lovelace-card.yaml`](https://www.google.com/search?q=https-%D0%B0%D0%B4%D1%80%D0%B5%D1%81-%D0%B2%D0%B0%D1%88%D0%B5%D0%B3%D0%BE-%D1%84%D0%B0%D0%B9%D0%BB%D0%B0-lovelace.yaml).
    *(**Рекомендация:** Создайте в репозитории файл `lovelace-card.yaml` и положите в него код карточки Mushroom для удобства пользователей)*.
